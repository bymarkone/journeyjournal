...
public class ReceiveGreatFeedbackCommand implements PhraseRuntimeCommand {
    @Override
    public void execute(TwDslRuntime twDslRuntime, PhraseToken phraseToken) {
        ...
        Feedback feedback = new Feedback(token.getDescription(), token.propertyTokenByName("from").get().getPropertyValue(), "Great");
